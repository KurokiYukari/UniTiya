...
{
    /// <summary>
    /// 修改自 <see cref="Cinemachine.CinemachineInputProvider"/>
    /// 原来的脚本会自动 enable InputAction，这里把这个功能删除。
    /// 应该添加到 ViewController 的 FreeLookVC 以重载输入。
    /// 不手动添加也可以（TiyaViewController 会自动添加），但可能会有一个（没有实际影响的）Error Log。
    /// </summary>
    [AddComponentMenu(TiyaTools.UniTiyaName + "/View/VC Input Provider")]
    class VCInputProvider : MonoBehaviour, Cinemachine.AxisState.IInputAxisProvider
    {
        /// <summary>
