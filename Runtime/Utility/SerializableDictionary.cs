// 修改自 Mathieu Le Ber 的 SerializedableDictionary
// https://assetstore.unity.com/packages/tools/integration/serializabledictionary-90477 
// 该版本中修复了原本在处理自定义 PropertyDrawer 类时可能会产生的 inspector GUI 重叠问题。
// 详细见 UniTiya\Editor\Utility\SerializableDictionaryPropertyDrawer.cs 的 CanPropertyBeExpanded 方法。
//
...
namespace Sarachan.UniTiya.Utility
{
	public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : Dictionary<TKey, TValue>, ISerializationCallbackReceiver
	{
		...
		internal SerializableDictionaryBase()
		{
		}

		internal SerializableDictionaryBase(IDictionary<TKey, TValue> dict) : base(dict.Count)
		{
			CopyFrom(dict);
		}
