...
namespace Sarachan.UniTiya.TiyaAnimator
{
    ...
    public abstract class WeaponTiyaAnimatorAdapterBase : MonoBehaviour, IWeaponTiyaAnimatorAdapter
    {
        ...
        protected void Update()
        {
            ...
            {
                TiyaAnimatorTools.AllLayerWeightLerper(OwnerAnimator, CurrentTiyaAnimatorLayer, TiyaAnimatorTools.Layer.WeaponFullBody);
                ...
                if (TiyaAnimatorTools.WeaponLayerStates.IsInNormalAttackStates(OwnerAnimator))
                {
                    ...
                    OwnerAnimator.SetLayerWeight(CurrentTiyaAnimatorLayer.ToLayerIndex(OwnerAnimator), 1);
                }

                for (int i = 0; i < ExtraActionAnimationBindList.Count; i++)
                {
                    // TODO: 这里会有延时，导致层的切换慢于动画的开始
                    if (OwnerAnimator.GetCurrentAnimatorStateInfo(TiyaAnimatorTools.Layer.WeaponFullBody.ToLayerIndex(OwnerAnimator)).IsName($"Action.Action {i + 1}"))
                    {
